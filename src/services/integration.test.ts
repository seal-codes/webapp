...
beforeAll(async () => {
  ...
  // Load your WASM file
  const wasmPath = path.join(__dirname, "../../public/main.wasm"); // Adjust path
...
describe("Integration Tests", () => {
  ...
  describe("Document Hash Calculation", () => {
    it("should calculate hashes for a real image file using actual DocumentHashService logic", async () => {
      console.log(
        "Testing real image hash calculation using DocumentHashService..."
      );

      // Load the test image from filesystem
      const imagePath = join(
        process.cwd(),
        "src/test-artifacts/sample-picture.jpeg"
      );
      ...
      try {
        ...
        const documentHashes = await (
          hashService as DocumentHashServiceWithPrivates
        ).calculateHashesFromImageData(
        ...
        console.log(
          "Real image hash calculation test using actual service completed successfully!"
        );
      } catch (error) {
  ...
  describe("Complete Roundtrip: Seal and Verify Workflow", () => {
    it("should seal a document and then successfully verify the sealed document", async () => {
      ...
      console.log("Sealing parameters defined:", {
        exclusionZone,
        userIdentity,
        serviceInfo,
        userUrl,
      ...
      const decodedAttestationJson = JSON.stringify(
        decodedData.attestationData
      );
      console.log(
        "Original attestation data length:",
        originalAttestationJson.length
      );
      console.log(
        "Decoded attestation data length:",
        decodedAttestationJson.length
      );
      ...
      const sealedCanvas = createCanvas(
        sealingImage.width,
        sealingImage.height
      );
      ...
      const sealedImageFile = new File(
        [sealedImageBuffer],
        "sealed-sample.png",
        { type: "image/png" }
      );
      console.log(
        "Sealed document file created:",
        sealedImageFile.name,
        "Size:",
        sealedImageFile.size,
        "bytes"
      );
      ...
      const verificationCanvas = createCanvas(
        sealedImage.width,
        sealedImage.height
      );
      ...
      const verificationImageData = verificationCtx.getImageData(
        0,
        0,
        verificationCanvas.width,
        verificationCanvas.height
      );
      const verificationHashes = await (
        hashService as DocumentHashServiceWithPrivates
      ...
      const cryptographicMatch =
        verificationHashes.cryptographic === originalHashes.cryptographic;
      const perceptualMatch =
        verificationHashes.pHash === originalHashes.pHash &&
      ...
      expect(verificationHashes.cryptographic).toBe(
        originalHashes.cryptographic
      );
