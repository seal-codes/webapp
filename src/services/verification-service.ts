 ...
 */
export function getExclusionZone(
  decodedData: DecodedVerificationData | null
): QRCodeExclusionZone | undefined {
...
export class VerificationService {
  ...
  encodeForQR(attestationData: AttestationData): string {
    try {
      ...
      console.log(
        "📦 Original attestation data size:",
        JSON.stringify(attestationData).length,
        "chars"
      );
      console.log(
        "📦 Compact data size:",
        JSON.stringify(compactData).length,
        "chars"
      );
      ...
      console.log(
        "📦 Compression ratio:",
        Math.round(
          (1 - base64url.length / JSON.stringify(attestationData).length) * 100
        ),
        "%"
      );
   ...
   */
  private createUltraCompactData(
    attestationData: AttestationData
  ): UltraCompactData {
  ...
  async verifyDocumentIntegrity(
    ...
    try {
      ...
      const exclusionZone =
        attestationBuilder.extractExclusionZone(attestationData);

      // Calculate hashes for the uploaded document with exclusion zone
      const calculatedHashes =
        await documentHashService.calculateDocumentHashes(
          document,
          exclusionZone
        );
      ...
      if (!cryptographicMatch && !perceptualMatch) {
        console.log(
          "❌ Document hashes do not match - document has been modified"
        );
        return {
          isValid: false,
          status: "modified",
      ...
      console.log(
        "✅ Document hashes verified, proceeding with signature verification..."
      );
      ...
      if (attestationData.sig) {
        ...
        signatureVerification =
          await signatureVerificationService.verifySignature(attestationData);
  ...
  getEncodingStats(attestationData: AttestationData): {
    ...
    const compressionRatio =
      (1 - finalEncoded.length / originalJson.length) * 100;
    ...
    if (attestationData.u && attestationData.u.length > 20) {
      recommendations.push(
        "User URL is long - consider shortening or removing"
      );
    }
